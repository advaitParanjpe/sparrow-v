...
module rv32_vec_vadd_engine #(
  ...
  localparam logic [3:0] VADD8_OP = 4'd3, VDOT8_OP = 4'd4;
  ...
  logic [4:0] vd_q; logic [VLEN-1:0] result_q; logic vector_write_q, store_write_q;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      vd_q <= 0; result_q <= 0; vector_write_q <= 0; store_write_q <= 0; scalar_result_q <= 0; store_addr_q <= 0; store_data_q <= 0; status_q <= 0; cause_q <= 0; pending_q <= 0; cpl_hold_q <= 0; cpl_hold_q_payload <= 0;
    end else begin
      ...
      if (vec_cmd_valid && vec_cmd_ready) begin
        vd_q <= vec_cmd_vd; vector_write_q <= 0; store_write_q <= 0; scalar_result_q <= 0; status_q <= 0; cause_q <= 0;
