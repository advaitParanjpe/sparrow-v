...
module rv32_core_pipe #(
 ...
 input logic vec_cpl_valid,output logic vec_cpl_ready,input logic vec_cpl_id,input logic [1:0] vec_cpl_status,input logic vec_cpl_result_valid,input logic [31:0] vec_cpl_result_data,vec_cpl_exception_cause
  ...
  // 111=VSDOT8. VSDOT8 reserves instr[28:25], and uses instr[31:29] as
  // metadata; malformed reserved bits remain scalar-illegal.
  assign if_vec=(if_ins[6:0]==7'h0b)&&((if_ins[14:12]==3'd0)||(if_ins[14:12]==3'd1)||(if_ins[14:12]==3'd2)||(if_ins[14:12]==3'd3)||(if_ins[14:12]==3'd4)||(if_ins[14:12]==3'd5)||(if_ins[14:12]==3'd6)||((if_ins[14:12]==3'd7)&&(if_ins[28:25]==4'd0)));
  ...
  assign vec_cmd_rs1_valid=1'b1; assign vec_cmd_rs2_valid=1'b1; assign vec_cmd_rd=dx_rd; assign vec_cmd_rd_we=(dx_vec_funct3==3'd0)||(dx_vec_funct3==3'd4)||(dx_vec_funct3==3'd7);
  ...
  always_ff @(posedge clk) begin
    if(!rst_n) begin
      ...
      if(vec_cmd_fire) begin
        vec_outstanding<=1; vec_payload_valid<=0; vec_pc_q<=dx_pc; vec_ins_q<=dx_ins; vec_rd_q<=dx_rd; vec_rd_we_q<=(dx_vec_funct3==3'd0)||(dx_vec_funct3==3'd4)||(dx_vec_funct3==3'd7); dx_v<=0;
      end
